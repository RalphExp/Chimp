...
func (vm *VM) Run() error {
	...
	for vm.currentFrame().ip < len(vm.currentFrame().Instructions())-1 {
		...
		case code.OpEqual, code.OpNotEqual,
			code.OpLess, code.OpLessEqual,
			code.OpGreater, code.OpGreaterEqual:
		...
		case code.OpReturnValue:
			...
			err := vm.push(returnValue) // returnValue overwrite the function object
			if err != nil {
				return err
			}
		...
		case code.OpReturn:
			...
			err := vm.push(Null) // returnValue overwrite the function object
			if err != nil {
				return err
			}
